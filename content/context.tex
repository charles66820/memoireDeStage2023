...
\subsubsection{TODO: trouvé un titre}
...
La carte BXI est capable de faire progressé le communication réseau sans aucune intervention du CPU (offload des communications réseau). % offloading des tâches réseau ?
...
C'est pour cela que la progression peut ce faire au niveau des fonctions \code{MPI_Wait} et \code{MPI_Test} ou être fait avant est donc l'appel aux fonctions s'occupe juste de la terminaison.
...
\begin{itemize}
  ...
  Quand il n'y à plus de calcul à faire on repasse à une progression bloquante par un appel à \code{MPI_Wait} sauf si la communication est déjà fini.
